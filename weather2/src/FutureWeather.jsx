            ...
            <div className="future-hours">
                {forecastData.map((forecast, index) => (
                    <div key={index} className="weather-slot">
                        <p className="next-hours">{new Date(forecast.dt * 1000).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                        <p className="next-temperature">{Math.round(forecast.main.temp - 273.15)}°C</p>
                        <img alt="icon-condition" src={getWeatherIcon(forecast.weather[0].main)} />
                    </div>
                ))}
            </div>
        </section>
